...
public class PointsToInitializer {
	...
	private void analyzeAllInitializers() {
		for (SootMethod method : this.c.getMethods()) {
			...
			for (Unit u : method.getActiveBody().getUnits()) {
				// go through all units which are invoke statements for new store

				if (u instanceof JInvokeStmt) {
					JInvokeStmt j = (JInvokeStmt) u;
					InvokeExpr ie = j.getInvokeExpr();

					// populate perMethod
					if (ie instanceof JSpecialInvokeExpr) {
						if (isRelevantInit((JSpecialInvokeExpr) ie)) {
							List<Value> args = ie.getArgs();
							if (!(args.get(0) instanceof IntConstant) || !(args.get(1) instanceof IntConstant)) {
								System.out.println("not implemented");
							}
							int trolley_size = ((IntConstant) args.get(0)).value;
							int reserve_size = ((IntConstant) args.get(1)).value;

							StoreInitializer si = new StoreInitializer(j, uniquenum++, trolley_size, reserve_size);
							perMethod.put(method, si);
							// populate initializers
							Collection<Node> nodes = getAllocationNodes((JSpecialInvokeExpr) ie);
							for (Node n : nodes) {
								initializers.put(n, si);
							}
						}
					}
				}
	 ...
	 * invokeExpression, which must be a call to Store init function
	...
	public Collection<Node> getAllocationNodes(JSpecialInvokeExpr invokeExpr) {
		if (!isRelevantInit(invokeExpr)) {
			throw new RuntimeException("Call to getAllocationNodes with " + invokeExpr.toString()
					+ "which is not an init call for the Store class");
		}
